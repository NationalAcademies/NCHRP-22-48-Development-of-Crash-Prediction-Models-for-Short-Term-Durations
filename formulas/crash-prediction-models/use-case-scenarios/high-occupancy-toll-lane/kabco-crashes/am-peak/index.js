...
function specialModelFormula(row) {
  ...
  res = Number(row.crash_year) * Number(row.Miles) * Math.exp(
    -4.640 + 1.345 * Math.log(Number(row.GP_Volume_UP))
    -1.695 * Math.log(row.GP_AvgSpeed)
    + 0.105 * (row.GP_AvgSpeed - row.GP_AvgSpeed_DOWN)
...
function genericModel(row, factor) {
  ...
  res = Number(row.crash_year) * Number(row.Miles) * Math.exp(
    ...
    + 0.636 * row.LaneNumber3_4 + 1.290 * row.LaneNumber5_) * factor
  ;
  return res;
}
